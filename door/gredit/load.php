    ...
    if ($arg[0] === 'group') {
            ...
            if (!empty($arg[1]['name'])) {
                ...
                if ($cr == 0) {
                    ...
                    if (!empty($arg[1]['description'])) {
                        $gdescp = db('Grupo', 's', 'options', 'type,v1', 'gdescp', $arg[1]["id"]);
                        if (count($gdescp) > 0) {
                            db('Grupo', 'u', 'options', 'v2,tms', 'type,v1', $arg[1]['description'], dt(), 'gdescp', $arg[1]["id"]);
                        } else {
                            db('Grupo', 'i', 'options', 'type,v1,v2,tms', 'gdescp', $arg[1]["id"], $arg[1]['description'], dt());
                        }
                    }
        ...
        if (gr_role('access', 'sys', '6')) {
            ...
            if (!empty($arg[1]['ltext']) && !empty($arg[1]['url']) && !empty($arg[1]['id'])) {
                ...
                gr_prnt('say("'.$GLOBALS["lang"]->updated.'","s");window.location.href = "";$(".grupo-pop").fadeOut();');
            } else {
                gr_prnt('say("'.$GLOBALS["lang"]->invalid_value.'");');
            }
        }
        ...
        if (usr('Grupo', 'alter', 'email', $arg[1]['email'], $arg[1]['id']) || usr('Grupo', 'select', $arg[1]['id'])['email'] == $arg[1]['email']) {
            if (usr('Grupo', $nonlatreg, 'name', $arg[1]['user'], $arg[1]['id']) || usr('Grupo', 'select', $arg[1]['id'])['name'] == $arg[1]['user']) {
                ...
                if ($GLOBALS["default"]->non_latin_usernames == 'enable') {
                    if (strpos($arg[1]['user'], ' ') !== false) {
                        ...
                        $arg[1]['user'] = strtolower($arg[1]['user']);
                    }
                } else {
                    $arg[1]['user'] = strtolower(vc($arg[1]['user'], 'alphanum'));
                }
                if (empty($arg[1]['user'])) {
                    gr_prnt('say("'.$GLOBALS["lang"]->invalid_value.'");');
                } else {
