...
function ses($d, $t = 0, $v = 0, $xt = 0) {
    ...
    if ($t === 'del') {
        ...
        $sesdev = ip().ip('dev');
        $sescode = rn("5").rn("9");
        $sesid = db($d, 'i', 'session', 'uid,device,code,tms', $v, $sesdev, $sescode, dt());
        if (!empty($xt)) {
            $ctimer = time() + (86400 * $xt);
        }
        setcookie($d.'usrdev', $sesdev, $ctimer, "/");
        setcookie($d.'usrcode', $sescode, $ctimer, "/");
        setcookie($d.'usrses', $sesid, $ctimer, "/");
        return true;
    } else {
        $r['active'] = false;
        $r['id'] = 0;
        if (isset($_SESSION[$d.'usrses']) && isset($_SESSION[$d.'usrcode']) && isset($_SESSION[$d.'usrdev'])) {
            setcookie($d.'usrdev', $_SESSION[$d.'usrdev'], 0, "/");
            setcookie($d.'usrcode', $_SESSION[$d.'usrcode'], 0, "/");
            setcookie($d.'usrses', $_SESSION[$d.'usrses'], 0, "/");
