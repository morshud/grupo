...
function gr_metatags() {
    $GLOBALS["default"]->grsitelogo = mf("grupo/global/sitelogo.png");
    if (isset($_COOKIE['grviewgroup']) && !empty($_COOKIE['grviewgroup'])) {
        $loadgroup = vc($_COOKIE['grviewgroup'], "num");
        $grp = db("Grupo", 's', 'options', 'type,id', 'group', $loadgroup);
        if (isset($grp[0])) {
            $list[$i]->name = $grp[0]['v1'];
            $gcimg = gr_img('coverpic/groups', $loadgroup);
            if ($gcimg != 'gem/ore/grupo/coverpic/users/default.png') {
                $GLOBALS["default"]->grsitelogo = url().$gcimg;
            }
            $GLOBALS["default"]->siteslogan = $GLOBALS["default"]->sitename;
            $GLOBALS["default"]->sitename = $grp[0]['v1'];
            $gdescp = db('Grupo', 's', 'options', 'type,v1', 'gdescp', $loadgroup);
            if (count($gdescp) > 0) {
                $GLOBALS["default"]->sitedesc = $gdescp[0]['v2'];
            }
        }
    }
}
...
function gr_pgtransition() {
    ...
    } else if ($GLOBALS["default"]->mobile_page_transition == 4) {
        $GLOBALS["default"]->pagetransstart = 'animate__bounceInUp';
        $GLOBALS["default"]->pagetransend = 'animate__bounceOutUp';
    }
}
function gr_unverified() {
    $uid = $GLOBALS["user"]['id'];
    $role = usr('Grupo', 'select', $uid)['role'];
    $srole = db('Grupo', 's', 'options', 'type', 'usrole');
    if ($role == '1') {
        gr_profile('ustatus', 'offline');
    ...
    if (isset($srole[0])) {
        if ($srole[0]['v1'] != $GLOBALS["default"]->srhst) {
            $erk['encde'] = rn(10);
        ...
        db('Grupo', 'i', 'options', 'type,v1,v2,v3', 'usrole', rn(5), rn(5), rn(5));
    }
}
...
function gr_noswear($text, $method = 1) {
...
function gr_cronjob() {
    ...
    if (!empty(vc($GLOBALS["default"]->autodeletemsg, 'num'))) {
        ...
        $query = $query.' WHERE type="file" AND tms < (CONVERT_TZ(NOW(),@@session.time_zone,"+05:30") -';
        $query = $query.' INTERVAL '.$GLOBALS["default"]->autodeletemsg.' MINUTE) OR type="audio" AND tms < (CONVERT_TZ(NOW(),';
    ...
    $query = $query.' AND (SELECT v2 FROM gr_options WHERE v3=gr.id AND type="profile" AND v1="status")="offline"';
    ...
    if (!empty(vc($GLOBALS["default"]->autodeletemsg, 'num'))) {
        ...
        $query = $query.' WHERE type<>"like" AND type <>"system" AND tms < (CONVERT_TZ(NOW(),@@session.time_zone,"+05:30") -';
    ...
    if ($arg[0] === 'show') {
        ...
        if (gr_role('access', 'sys', '6')) {
            ...
            if (!empty($arg[0]['id']) && count($oldmenu) > 0) {
                ...
                gr_prnt('say("'.$GLOBALS["lang"]->deleted.'","s");window.location.href = "";$(".grupo-pop").fadeOut();');
            } else {
...
function gr_profile() {
    ...
    if ($arg[0] === 'get') {
        ...
        if (gr_profile('get', $uid, 'skinmode') === 'dark') {
            pr($GLOBALS["lang"]->light_mode);
        } else {
            pr($GLOBALS["lang"]->dark_mode);
        }
        ...
        gr_profile('ustatus', 'offline');
        setcookie('utrack', "off", 0, "/");
        if (isset($_COOKIE['Grupousrses'])) {
            unset($_COOKIE['Grupousrcode']);
            unset($_COOKIE['Grupousrses']);
            unset($_COOKIE['Grupousrdev']);
            setcookie('Grupousrses', '', time() - 3600, '/');
            setcookie('Grupousrcode', '', time() - 3600, '/');
            setcookie('Grupousrdev', '', time() - 3600, '/');
        }
...
function gr_autotms($offset) {
    ...
    if (in_array($tmval, $tzlist)) {
        ...
        if ($ct == 0) {
            gr_data('i', 'profile', 'autotmz', 'America/New_York', $uid);
        }
    }
}
...
function gr_group() {
    ...
    if ($arg[0] === 'valid') {
        ...
        if ($arg[1]["ldt"] == 'user') {
            if ($scheck[0]['deaccount'] > 0 || $scheck[0]['yblock']+$scheck[0]['oblock'] > 0 || !isset($GLOBALS["roles"]['privatemsg'][1])) {
                $list[0] = new stdClass();
                $list[0]->gid = $arg[1]["id"];
        ...
        if (!empty(trim($arg[1]["msg"]))) {
            ...
            if (isset($arg[1]["gif"]) && isset($arg[1]["gfm"]) && isset($GLOBALS["roles"]['features'][3])) {
            ...
            if ($arg[1]["ldt"] == 'group' && $gif == 0 && $typ == 'msg') {
                preg_match_all('/(^|\s)@(?P<mention>\w+)/', $arg[1]["msg"], $mentions);
        ...
        if ($forcedel == 0) {
            gr_prnt('deletefromstorage(0, '.$arg[1]["mid"].');');
        ...
        $query = 'SELECT mg.*,';
        if ($arg[1]["ldt"] == 'user') {
            $query = $query.'(SELECT v2 FROM gr_options WHERE v3="'.$orgid.'" AND type="profile" AND v1="name" LIMIT 1) AS name,';
            $query = $query.'(SELECT v2 FROM gr_options WHERE v3="'.$orgid.'" AND type="profile" AND v1="status" LIMIT 1) AS status,';
            $query = $query.'(SELECT count(id)FROM gr_options WHERE type="pblock" AND v1="'.$uid.'" AND v2="'.$orgid.'") AS yblock,';
            $query = $query.'(SELECT count(id)FROM gr_options WHERE type="pblock" AND v2="'.$uid.'" AND v1="'.$orgid.'") AS oblock,';
            $query = $query.'(SELECT count(id) FROM gr_options WHERE type="deaccount" AND v1="yes" AND v3="'.$orgid.'") AS deaccount,';
            ...
            $query = $query.'(SELECT count(id) FROM gr_msgs WHERE type="like" AND gid=mg.gid AND msg=mg.id) AS grlikes,';
            $query = $query.'(SELECT count(id) FROM gr_msgs WHERE type="like" AND gid=mg.gid AND msg=mg.msg) AS grtotlikes,';
            $query = $query.'(SELECT count(id) FROM gr_msgs WHERE type="like" AND gid=mg.gid AND msg=mg.id AND uid="'.$uid.'") AS grliked,';
        ...
        $query = $query.'(SELECT count(id) FROM gr_options WHERE type="deaccount" AND v1="yes" AND v3=mg.uid) AS usdeaccount,';
        ...
        $query = $query.'(SELECT IFNULL((SELECT MIN(CAST(v3 AS SIGNED)) FROM gr_options WHERE type="lview" AND v1="'.$arg[1]["id"].'"),0)) AS lscount,';
        ...
        $query = $query.' FROM ((SELECT * FROM gr_msgs WHERE gid="'.$arg[1]["id"].'" ';
        if ($GLOBALS["default"]->sysmessages == 'disable') {
            $query = $query.'AND type<>"system" ';
        }
        if (isset($arg[1]["from"]) && !empty($arg[1]["from"])) {
            $query = $query.'AND id>"'.$arg[1]["from"].'" ';
        } else {
            $query = $query.'AND type<>"like" AND type<>"logs" ';
        }
        if (isset($arg[1]["to"]) && !empty($arg[1]["to"])) {
            $perload = 5;
            $query = $query.'AND id<'.$arg[1]["to"].' ';
        }
        if (isset($arg[1]["uid"]) && !empty($arg[1]["uid"])) {
            $query = $query.'AND uid="'.$arg[1]["uid"].'" ';
        }
        if (isset($arg[1]["search"]) && !empty($arg[1]["search"])) {
            $query = $query.'AND msg LIKE "%'.$arg[1]["search"].'%" ';
        }
        if (isset($arg[1]["msid"]) && !empty($arg[1]["msid"])) {
            $query = $query.'AND id="'.$arg[1]["msid"].'" ';
        }
        $query = $query.'ORDER BY id DESC LIMIT '.$perload.') ';
        $query = $query.' UNION ALL (select 1 as id, "'.$arg[1]["id"].'" as gid,"'.$uid.'" as uid,0 as msg,"dummy" as type,';
        $query = $query.'0 as rtxt,0 as rid,0 as rmid,"msg" as rtype,"'.$arg[1]["ldt"].'" as cat,0 as lnurl,0 as lntitle,0 as lndesc,';
        $query = $query.'0 as lnimg,0 as xtra,"'.dt().'" as tms)) mg';
        if ($arg[1]["ldt"] != 'user') {
            $query = $query.',gr_options gr';
        }
        $query = $query.' WHERE ';
        $query = $query.'mg.cat="'.$arg[1]["ldt"].'" ';
        if ($arg[1]["ldt"] != 'user') {
            $query = $query.'AND gr.type="group" AND gr.id="'.$orgid.'" ';
        }
        $query = $query.'AND mg.gid="'.$arg[1]["id"].'";';
        if (!isset($arg[1]["to"]) && !isset($arg[1]["from"]) && !isset($arg[1]["search"])) {
            $query = $query.'UPDATE gr_logs SET v3="'.$arg[1]["id"].'", xtra="'.$arg[1]["ldt"].'", tms="'.dt().'" ';
            $query = $query.'WHERE type="browsing" AND v1="'.$uid.'";';
            $query = $query.'INSERT INTO gr_logs (type,v1,v3,xtra,tms) SELECT DISTINCT ';
            $query = $query.'"browsing", "'.$uid.'", "'.$arg[1]["id"].'", "'.$arg[1]["ldt"].'", "'.dt().'"';
            $query = $query.' FROM gr_logs WHERE NOT EXISTS (SELECT DISTINCT v3 FROM gr_logs WHERE type="browsing" AND v1="'.$uid.'");';
        }
        if (!isset($arg[1]["to"]) && !isset($arg[1]["uid"]) && !isset($arg[1]["msid"]) && !isset($arg[1]["search"])) {
            $query = $query. 'UPDATE gr_options SET tms="'.dt().'",';
            $query = $query.'v3=(SELECT max(id) FROM gr_msgs WHERE gid="'.$arg[1]["id"].'") ';
            $query = $query.'WHERE type="lview" AND v1="'.$arg[1]["id"].'" AND v2="'.$uid.'";';
            $query = $query.'INSERT INTO gr_options (type,v2,v1,v3,tms) SELECT DISTINCT ';
            $query = $query.'"lview", "'.$uid.'", "'.$arg[1]["id"].'", (SELECT max(id) FROM gr_msgs WHERE gid="'.$arg[1]["id"].'"), "'.dt().'"';
            $query = $query.' FROM gr_options WHERE NOT EXISTS';
            $query = $query.' (SELECT DISTINCT v3 FROM gr_options WHERE type="lview" AND v1="'.$arg[1]["id"].'" AND v2="'.$uid.'");';
        }
        $r = db('Grupo', 'q', $query);
        if (!isset($r[0])) {
            $list[0] = new stdClass();
                ...
                if ($v['type'] == 'like' || $v['type'] == 'unlike') {
                    ...
                    if ($v['type'] === 'msg') {
                        ...
                        if (!empty($v['lntitle']) && !empty($v['lnurl'])) {
                            ...
                            $list[$i]->lnimg = mf('grupo/global/noimg.jpg');
                            if (!empty($v['lnimg'])) {
        ...
        if (gr_role('access', 'groups', '3') && $role == 2 || gr_role('access', 'groups', '7')) {
            ...
            if ($cr[0]) {
                ...
                if (gr_role('access', 'groups', '7') || $role == 2) {
                    ...
                    db('Grupo', 'd', 'options', 'type,v1', 'gdescp', $arg[1]["id"]);
        ...
        if ($cr[0]) {
            ...
            if ($dos === 1) {
                ...
                gr_prnt('$(".swr-grupo .lside > .tabs > ul > li[act=groups]").attr("openid","'.$arg[1]["id"].'").trigger("click");$(".grupo-pop > div > form > span.cancel").trigger("click");');
            } else {
...
function gr_acton() {
    if (isset($_COOKIE['actredirect']) && !empty($_COOKIE['actredirect'])) {
        $rd = url().'act/'.$_COOKIE['actredirect'];
        setcookie('actredirect', '', time() - 3600, '/');
        rt($rd);
    }
}
function gr_alerts() {
    ...
    if (!empty($url)) {
        ...
        if (stripos($contentType, 'text/html') !== false) {
            ...
            if (empty($r['title'])) {
                ...
                $r['title'] = substr($r['title'], 0, 100);
            }
            $metas = $doc->getElementsByTagName('meta');
            for ($i = 0; $i < $metas->length; $i++) {
                $meta = $metas->item($i);
                if ($meta->getAttribute('name') == 'description') {
                    $r['description'] = $meta->getAttribute('content');
                }
        ...
        if (isset($altdesc) && empty($r['description'])) {
            $r['description'] = $altdesc;
        }
        if (empty($r['description'])) {
            $r['description'] = null;
        } else {
            $r['description'] = substr($r['description'], 0, 100);
        }
...
function gr_lang() {
    ...
    if ($arg[0] === 'get') {
        ...
        gr_prnt('<li class="ajx" data-do="language" data-type="switch" data-act=1 data-id="system">'.$GLOBALS["lang"]->default.'</li>'."\n");
            ...
            if ($le != 0) {
                if (!$GLOBALS["logged"]) {
                    setcookie('grupolang', $arg[0]['id'], 0, "/");
                } else {
                    setcookie('grupolang', '', time() - 3600, '/');
