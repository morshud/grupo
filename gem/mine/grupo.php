<?php if(!defined('s7V9pz')) {die();}?>var grversion = 2.5;
var alertitle;
var gruploader;
var grlzyload;
var gremojiz;
var grlocalstorage = 1;
...
function loadlist(el, e) {
    ...
    s = s+'list=list+"</u><span>"+data[k].sub+"</span></span><span class=right>';
    ...
    s = s+'if(data[k].od!==undefined){if(data[k].od!==0){list=list+"<li "+data[k].odt+">"+data[k].od+"</li>";}}';
...
$('body').on('click', '.swr-grupo .loadgroup,.dumb .loadgroup', function(e) {
    var grkey = 'grp'+$(this).attr("ldt")+$(this).attr('no');
    if (grkey in localStorage && $(this).find('.cnts').text() != "") {
        localStorage.removeItem(grkey);
    }
...
$('body').on('click', '.swr-grupo .goback', function(e) {
    if ($(window).width() <= 767.98) {
        ...
        if (block == 'alerts' || block == 'rside') {
            ...
            $(".swr-grupo .rside").addClass("animate__animated "+animclose+" animate__fast");
            setTimeout(function() {
                $('.swr-grupo .rsides').addClass('nomob');
                $('.swr-grupo .rsides').removeClass('abmob');
                $('.swr-grupo .panel').removeClass('bwmob');
                $('.swr-grupo .panel').addClass('abmob');
            }, 500);
        } else {
...
function loadgroup($id, e, r) {
    ...
    if ($id != $('.swr-grupo .panel').attr('no') || $id == $('.swr-grupo .panel').attr('no') && e.attr('ldt') != $('.swr-grupo .panel').attr('ldt') || r == 1 || e.attr('msgload') != undefined) {
        ...
        var grkey = 'grp'+ldt+$id;
        if (grkey in localStorage) {
            var olddata = JSON.parse(localStorage.getItem(grkey));
            loadgrdata(e, olddata);
            ...
            ajxvar['loadgroup'] = $.ajax({
                ...
                if (grlocalstorage == 1) {
                    localStorage.setItem('grp'+ldt+data[0].gid, JSON.stringify(data));
                }
                loadgrdata(e, data);
...
function loadgrdata(e, data) {
    ...
    $(".swr-grupo .groupnav > .left > span > span").html(htmlDecode(data[0].pntitle)+"<span>"+data[0].pnsub+"</span><ul class=typing tcount=0></ul>");
    ...
    $.each(data[1], function(k, v) {
        $(".swr-grupo .panel .swr-menu > ul").append("<li "+v[1]+">"+v[0]+"</li>");
    });
    if (e.attr('msgload') == undefined) {
        if ($(window).width() > 767.98 && $(".swr-grupo .panel").attr("ldt") != "user") {
...
function grsendmsg(el, e, gif, gfm, gfw, gfh) {
    ...
    if (gif != 0 && gfm != 0 && gfw != 0 && gfh != 0 && !el.hasClass('na') || $.trim(msgd) != '' && !el.hasClass('na')) {
        ...
        var msg = '<li class="you animate__animated animate__fadeIn '+senid+' '+moset+'" no="0"> <div>';
...
function loadmsg(d, n, fi) {
        ...
        if (k == 0 && d[0].messageflood == 1) {
            ...
            if (trn == 1) {
                ...
                if (m.type == 'like') {
                    ...
                    $('.swr-grupo .panel > .room > .msgs').append(msg);
                    likeintostorage(grkey, m.liked, m.total);
                } else if (m.type == 'dummy') {
                    var msg = '<li class="logs you dummy" no="'+m.id+'"> </li>';
                    $('.swr-grupo .panel > .room > .msgs').append(msg);
                } else if (m.type == 'logs') {
                    if (m.action == 'delete') {
                        deletefromstorage(grkey, m.rel);
                    ...
                    if (n == 2) {
                        ...
                        if (n == 0) {
                            if (grkey in localStorage) {
                                var newgdata = d;
                                appendToStorage(grkey, newgdata);
                            }
                        }
                        $('.swr-grupo .panel > .room > .msgs').append(msg);
                    }
    ...
    qrload();
    gremojiz = setTimeout(function() {
        if (gremojiz != undefined) {
            clearTimeout(gremojiz);
        }
        gremojibuild();
    }, 10);
...
function likeintostorage(grkey, skey, lcount) {
    if (grkey == 0) {
        grkey = 'grp'+$(".swr-grupo .panel").attr("ldt")+$(".swr-grupo .panel").attr("no");
    }
    var grpdata = localStorage.getItem(grkey);
    if (grpdata) {
        grpdata = JSON.parse(grpdata);
        $.each(grpdata, function(grpk, grpv) {
            if (grpdata[grpk] != null) {
                if (grpdata[grpk].id == skey) {
                    grpdata[grpk].lvn = nformat(lcount);
                    return false;
                }
            }
        });
        localStorage.setItem(grkey, JSON.stringify(grpdata));
    }
}

function deletefromstorage(grkey, skey) {
    if (grkey == 0) {
        grkey = 'grp'+$(".swr-grupo .panel").attr("ldt")+$(".swr-grupo .panel").attr("no");
    }
    var grpdata = localStorage.getItem(grkey);
...
$("body").on("keyup", ".swr-grupo .panel > .searchbar input", function(e) {
    if (e.which == 13) {
        ...
        if (search != "") {
            ...
            var s = '$(".swr-grupo .msgloader").fadeOut(100);if($(".swr-grupo .panel").attr("no")==data[0].gid){loadmsg(data,1);}';
...
function turn_chat($d, el) {
    ...
    if ($d === 'on') {
        $('.groupreload').fadeOut();
        $('.swr-grupo .panel > .textbox').removeClass('animate__animated animate__slideOutDown disabled');
        ...
        $(".swr-grupo .panel").attr('deactiv', 1);
        $('.swr-grupo .panel > .textbox').addClass('animate__animated animate__slideOutDown disabled');
    }
}
...
$('body').on('click tap touchstart', '.swr-grupo .vwp', function(e) {
    var kr = 3;
    var ths = $(this);
    var et = e;
    if ($(window).width() <= 767.98) {
        ...
        $(".swr-grupo .lside .opt > ul").hide();
        $(".grtab").addClass("d-none");
        if ($(this).parents('div.lside').length) {
            $('.swr-grupo .lside,.swr-grupo .panel').removeClass('abmob');
            $(".swr-grupo .lside,.swr-grupo .panel").addClass("bwmob");
            $('.swr-grupo .rside > .top > .left > .icon').attr('data-block', 'alerts');
    ...
    setTimeout(function() {
        ...
        var s = '$(".swr-grupo .aside > .content .profile > .top > span.name").text(htmlDecode(data[0].name));';
        ...
        s = s+'$(".swr-grupo .aside > .content .profile > .middle > span.stats > span").eq(0).find("i").text(data[0].mna);';
        s = s+'$(".swr-grupo .aside > .content .profile > .middle > span.stats > span").eq(1).find("i").text(data[0].mnb);';
        s = s+'$(".swr-grupo .aside > .content .profile > .middle > span.stats > span").eq(2).find("i").text(data[0].mnc);';
        ...
        s = s+'$(".swr-grupo .aside > .content .profile > .middle > span.pm").remove();';
        s = s+'$(".swr-grupo .aside > .content .profile > .middle").prepend("<span class='+"'pm'"+' "+data[0].tbattr+">"+data[0].btn+"</span>");';
        s = s+'$(".swr-grupo .aside > .content .profile > .middle > span.pm").addClass(data[0].tbclass);';
        ...
        s = s+'pbtm.append("<li><b>"+data[k].name+"</b><span>"+linkify(htmlDecode(data[k].cont))+"</span></li>");';
        s = s+'}});}$(".swr-grupo .rside > .content .profile").fadeIn();';
        s = s+'$(".swr-grupo .rside .listloader").fadeOut();$("[data-toggle=tooltip]").data("bs.tooltip",false).tooltip();';
        var f = '$(".swr-grupo .rside .listloader").addClass("error");';
        var grlv = 'ajxclrtm["grlivesetin"]=setTimeout(function() { gr_live(); }, 2000);';
        var grkey = 'grp'+$(".swr-grupo .panel").attr("ldt")+$(".swr-grupo .panel").attr("no");
        var grkyz = 'grlive';
        s = s+grlv;
        f = f+grlv;
        ajxx(ths, data, s, et, f, 'loadprofile', grkyz);
    }, 300);
});
...
function grpreview(ty, url, mime, vd) {
    ...
    if (ty == 'img') {
        ...
        if ($(window).width() <= 767.98) {
            elm.find('div > img').css("max-width", $(window).width() - 20 + "px");
            elm.find('div > img').css("max-height", $(window).height() - 20 + "px");
        } else {
    ...
    if (maxLength !== "") {
        if ($.trim(str).length > maxLength) {
            ...
            if (len == 0) {
                ...
                $shrt = $shrt+'<i class="moretext">' + gremojibuild(url2link(str, nl), ej) + '</i>';
            }
        } else {
            $shrt = gremojibuild(url2link(str, nl), ej);
        }
    } else {
        $shrt = gremojibuild(url2link(str, nl), ej);
    }
